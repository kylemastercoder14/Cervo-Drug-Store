...
const FeaturedProducts = () => {
  ...
  const calculateDiscountPercentage = (price: number, discountedPrice: number | null) => {
      ...
      {items.map((item) => {
        ...
        return (
          <div className="pt-5" key={item.id}>
            <div onClick={() => router.push(`/products/${item.tags}`)} className="bg-white cursor-pointer shadow-lg relative w-full h-[300px]">
              <Image src={item.image} alt="Featured" fill className="w-full h-full" />
              {discountPercentage !== null && (
                <div className="absolute top-3 flex items-start gap-1 left-2 px-2.5 py-4 rounded-full bg-[#437634]">
                  <p className="text-white font-black text-xl">{discountPercentage}</p>
            ...
            <div className="flex items-center gap-1 mt-2 text-sm">
              ...
              {item.discountedPrice !== null && (
                <p className="text-muted-foreground line-through">{formatPrice(item.discountedPrice)}</p>
              )}
            </div>
            <div className="flex items-center mt-2">
              <Button variant="primary" className="w-full" onClick={() => handleAddToCart(item)}>
                Add To Cart
              </Button>
